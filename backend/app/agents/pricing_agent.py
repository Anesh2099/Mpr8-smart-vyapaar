...
def pricing_agent(product_id: str, store_id: str = "store001") -> dict:
    """
    ...
    competitor = db.collection("competitor_prices").document(product_id).get().to_dict() or {}

    # ── Raw data ──────────────────────────────────────────────────────────────
    base_price = inv.get("price", 0)
    wholesale_cost = inv.get("wholesaleCost", base_price * 0.6)
    current_stock = inv.get("stock", 0)
    reorder_level = inv.get("reorderLevel", 10)
    product_name = inv.get("productName", product_id)

    predicted_demand = forecast.get("predictedDemand", 0)
    competitor_price = competitor.get("price", base_price)
    ...
    if seasonal_multiplier != 1.0:
        ...
        reasons.append(f"Seasonal factor for {datetime.now().strftime('%B')} (×{seasonal_multiplier})")

    # Factor 4: Competitor benchmark
    if competitor_price > 0:
        if base_price > competitor_price * 1.1:
            multiplier -= 0.05
            reasons.append(f"Competitor price is ₹{competitor_price} — discount to stay competitive")
        elif base_price < competitor_price * 0.9:
            multiplier += 0.05
            reasons.append(f"Competitor price is ₹{competitor_price} — slight increase possible")
    ...
    # ── Confidence score ──────────────────────────────────────────────────────
    data_quality = sum([
        bool(base_price), bool(predicted_demand), bool(competitor_price), bool(wholesale_cost)
    ])
...
def _generate_promotion(product_name: str, stock: int, demand: int, price: float) -> str | None:
    """Generate a promotion suggestion when overstock or near expiry."""
    if stock > 50 and demand < 10:
        return f"💡 Bundle Deal: Buy 3 {product_name} for ₹{round(price * 2.5, 0)} (save ₹{round(price * 0.5, 0)})"
    if demand > 50 and stock < 15:
        return f"🔥 Flash Sale Alert: Limited stock of {product_name} at ₹{price} — promote urgency!"
    return None